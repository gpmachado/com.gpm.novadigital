...
class SmartPlugBase extends TuyaZclBase {
  ...
  constructor(...args) {
    ...
    this._meterOffsetRaw  = 0;          // raw currentSummationDelivered subtracted by a software meter reset
  }
  ...
  async onNodeInit({ zclNode }) {
    ...
    this._meterOffsetRaw = this.getStoreValue('meterOffsetRaw') || 0;
  ...
  async _addMissingCapabilities() {
    for (const cap of ['measure_current', 'measure_voltage', 'button.reset_meter']) {
  ...
  _registerCapabilities() {
    ...
    if (this.hasCapability('button.reset_meter')) {
      this.registerCapabilityListener('button.reset_meter', () => this._onResetMeter());
    }
  }

  /**
   * Maintenance-action handler for button.reset_meter.
   *
   * Deliberately does NOT send Basic.factoryReset() (ZCL cmd 0x00). It's a
   * no-op on this device via Homey's zigbee-clusters (verified live), but a
   * live test of the same command through Zigbee2MQTT DID zero the meter —
   * and also knocked the device off the network, requiring remove + re-pair.
   * So "factory reset" on this hardware is a real factory reset, not a
   * metering-only one, whenever the stack actually gets it to execute.
   * Never send it from a maintenance button real users can click.
   *
   * Instead this is a pure software offset: store the current raw
   * currentSummationDelivered in the device store, and _parseMeter subtracts
   * it from every future reading. Nothing on the device itself changes —
   * meter_power just reads as if it started counting from here.
   */
  async _onResetMeter() {
    const ep = this.zclNode.endpoints[ENDPOINT_ID];

    const raw = await ep.clusters.metering.readAttributes(['currentSummationDelivered'])
      .then(r => r.currentSummationDelivered);

    this._meterOffsetRaw = raw;
    await this.setStoreValue('meterOffsetRaw', raw);
    await this.setCapabilityValue('meter_power', this._parseMeter(raw)).catch(this.error);
    this.log(`[Reset meter] software offset set to raw=${raw} — meter_power now 0`);
  }

  // ─── Parsers ───────────────────────────────────────────────────────────

  _parseMeter(raw) {
    const adjustedRaw = Math.max(0, raw - this._meterOffsetRaw);
    const kWh = (adjustedRaw / METERING_DIVISOR) * this._energyFactor;
