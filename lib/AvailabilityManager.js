...
class AvailabilityManager {
  ...
  static triggerRejoin(device, gapMs = 0) {
    ...
    device.log(`[AvailabilityManager] Firing flow: ${cardId} (gap ${Math.round(gapMs / 1000)}s)`);
    ...
    try {
      ...
      device.error('[AvailabilityManager] getNodeDevices error:', err.message);
    }
...
class AvailabilityManagerBase {
  ...
  async _markAlive(source) {
    try {
      ...
      if ((wasOffline || now - this._lastPersisted >= 60_000) && !this._persistingLastSeen) {
  ...
  async _pollDevice() {
    ...
    try {
      ...
      // readAttributes com timeout curto. Qualquer resposta (mesmo sem o
      // atributo específico — manufacturerName é opcional no ZCL Basic
      // cluster) já prova que o dispositivo respondeu na rede; exigir esse
      // atributo específico geraria falso-offline em devices que não o
      // implementam mas respondem normalmente.
      let timeoutId;
      let result;
      try {
   ...
   * Store is touched here only; the watchdog and _markAlive() read/write
   * `_lastSeen` directly afterwards.
   */
  async _seedLastSeen() {
   ...
   * pairing, or capability just added to an existing device). Homey persists
   * capability values across app restarts, so an existing false is left
   * alone here — forcing it true on every boot would fire a misleading
   * "turns on" trigger before the watchdog/frame hook actually confirms
...
class AvailabilityManagerPassive extends AvailabilityManagerBase {
  ...
  async _installHandleFrameHook() {
    ...
    if (node._availabilityHookInstalled) {
      ...
      // Zigbee node (the main/EP1 device — see _installAvailability() call
      // sites). A second instance here would run its own watchdog against a
      // node it never receives frames from, silently going stale while the
      // physical device stays online. Fail loudly instead of no-op'ing so a
      // future driver mistake surfaces in testing, not as a gang-goes-offline
      // bug in production.
      throw new Error(
        '[Availability] node.handleFrame already hooked — install AvailabilityManagerPassive '
        + 'on the main/EP1 device only; siblings get availability via _getSiblings() cascade.',
      );
    }
